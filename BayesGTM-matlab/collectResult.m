%% collect result
clear;clc;
genefiles = dir('results\results_processed\*.mat');

results = zeros(length(genefiles),2);
for i = 1:length(genefiles)
    s = genefiles(i).name;
    s = str2num(s(isstrprop(s,'digit')));
    filename = sprintf('results\\results_processed\\%s',genefiles(i).name);
    load(filename);
...
csvwrite('results\results_processed\results_processed.csv',results);